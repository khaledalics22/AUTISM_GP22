...
export class UserRepository extends BaseRepository<User>  {
    ...
    async updateEyeContact(userId, data: { date: string, score: number }) {
        const user = await this.getUserById(userId);
        if (!user.ScoreInDoing)
            await this.update(userId, { eyeContact: {} })
        if (!user.eyeContact[data.date]) {
            user.eyeContact[data.date] = {
                score: data.score,
                count: 1
            }
            await this.update(userId, { eyeContact: user.eyeContact })
        }
        else {
            user.eyeContact[data.date].score += data.score;
            user.eyeContact[data.date].count += 1;
            await this.update(userId, { eyeContact: user.eyeContact })
        }
    }
    ...
    async getStatisticsInEyeContact(userId) {
        ...
        var results = []
        var test = 1
        for (let i in user.eyeContact) {
            results.push({ "test_number": test, "score": (user.eyeContact[i].score / user.eyeContact[i].count) });
            test++;
        }
        return { results: results };
    }
    ...
    async getOfALLStatisticsInEyeContact() {
        var Total_result = [];
        var count = [];
        const users = await this.findAll();
        for (let j = 0; j < users.length; j++) {
            var user = users[j];
            let result_ = (await this.getStatisticsInEyeContact(user._id));
            let result = result_.results;
            for (let i = 0; i < result.length; i++) {
                if (Total_result.length <= i) {
                    Total_result.push(result[i]);
