...
export class UserController {
    ...
    @UseGuards(AuthGuard('jwt'))
    @Put('/me/eye_contact')
    async updateMyEyeContact(@Request() req, @Body() data: {
        date: string,
        score: string
    }) {
        let data_to = {
            date: data.date, score: parseFloat(data.score)
        }
        await this.userService.UpdateScoreToUserEyeContact(req.user._id, data_to)
    }

    @UseGuards(AuthGuard('jwt'))
    @Get('/me/recognize')
    async getMyEyeContact(@Request() req, @Query() data: {
        emotion: string,
        all: string
    }) {
    ...
    @UseGuards(AuthGuard('jwt'))
    @Get('/me/eye_contact')
    async getMyRecognizing(@Request() req) {
        return await this.userService.getScoreToUserEyeContact(req.user._id);
    }

    @UseGuards(AuthGuard('jwt'))
    @Get('/me/doing')
    async getEyeContact(@Request() req, @Query() data: {
        emotion: string,
        all: string
    }) {
        let data_to = { emotion: data.emotion, all: data.all == "true" ? true : false }
    ...
    @Get('/eye_contact')
    async getEyeContat() {
        return await this.userService.getAllUsersScoreInEyeContact();
    }
